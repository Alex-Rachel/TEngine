...
│
└── Editor/                        # Editor 代码生成工具 (放在 TEngine.Editor 程序集)
    ├── UITKBindingGenerator.cs    # 主生成器：扫描 [Q]/[OnClick]/[OnChange]/[Bind]/[BindCommand]
    ├── UITKNamingHelper.cs        # camelCase → kebab-case 转换
    └── UITKUXMLValidator.cs       # UXML 解析 + 元素校验
```
...
**不使用 Roslyn Source Generator DLL**，改为 Editor 脚本预生成：
- 通过 Unity Editor 菜单/保存时自动触发
- 扫描所有继承 UITKWindow/UITKWidget 的 `partial class`
- 解析 [Q]、[OnClick]、[OnChange]、[Bind]、[BindCommand] Attribute
- 生成 `XXX.bindgen.cs` 到同目录（普通 C# 文件，参与热更新）
- 可选：解析对应 UXML 文件校验元素名是否存在

### 优势
- 不需要构建外部 DLL
- 生成的 .cs 是普通代码，与 HybridCLR 热更完全兼容
- 零运行时反射开销
- 与现有 UIModule 的 ScriptGenerator 模式一致

### Editor 工具结构

```
Assets/Editor/UITKBindingGenerator/
├── UITKBindingGenerator.cs       # 主生成器：扫描类型、生成代码
├── UITKNamingHelper.cs           # camelCase → kebab-case
└── UITKUXMLValidator.cs          # UXML 解析 + 元素校验
```

### 触发方式
- 菜单：`TEngine/UITK/Generate All Bindings`
- 右键单文件：`TEngine/UITK/Generate Binding`
- 可选：保存时自动检测变更并重新生成
...
namespace GameLogic
{
    ...
    {
        private void __UITKAutoBind(UnityEngine.UIElements.VisualElement root)
        {
            btnLogin = root.Q<UnityEngine.UIElements.Button>("btn-login");
            lblTitle = root.Q<UnityEngine.UIElements.Label>("lbl-title");
        }

        private void __UITKAutoBindEvents()
        {
            btnLogin.clicked += OnBtnLogin;
        }
...
| 要点 | 措施 |
|------|------|
| 零运行时反射 | Editor 预生成绑定代码，运行时直接调用生成方法 |
...
| 代码生成需手动触发 | Editor 菜单 Generate Bindings 或保存时自动触发 |
